...
export default function codeReviewExtension(pi: ExtensionAPI) {
	...
	pi.registerCommand("review", {
		description:
			"Multi-model code review: uncommitted (default), this (session changes), stack, commit, paths, or a git commit/range. Flags: --auto (accept all defaults), --fix (send findings straight to the main agent), --cheap (single cheap model: muse-spark-contributor), --quick/--normal/--thorough (speed)",
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			cheap: Type.Optional(
				Type.Boolean({
					description: "When true, run all reviewers with just llm-jrodal/muse-spark-contributor (cheap). Equivalent to --cheap on /review.",
				}),
			),
		}),
